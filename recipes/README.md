...
| Check | Status |
|---|---|
| Every `--flag` in a recipe's argv is listed by `apr <verb> --help` on the release binary | **Checked** by `scripts/check_recipe_argv_surface.py`: 8/8 against the published `apr 0.69.1` (release asset, prints `v0.69.1+no-git`). A misspelt flag fails, and that negative control was run |
