...
impl LossScaler {
    ...
    fn unscale_grad(&self, grad: f32) -> f32 {
        grad / self.scale
    }

    fn update(&mut self, has_overflow: bool) {
        if has_overflow {
...
fn cross_entropy(output: &[f32], target: usize) -> f32 {
