//! # Opt Smoke Inference
//!
//! Load a synthetic opt micro-checkpoint via `aprender::rosetta`,
...
//! Provable-contract: contracts/inference-opt-smoke-v1.yaml (grade A; lean_status: wip)
//! Citation: Zhang et al. (2022). OPT: Open Pre-trained Transformer Language Models. arXiv:2205.01068
//!
//! Run with: cargo run --example inference_opt_smoke
//!
//! Added by PMAT-300+ (architecture-demos: opt coverage).
...
pub fn smoke(fixture_path: &str, format: &str) -> SmokeVerdict {
    ...
    SmokeVerdict::LoaderUnavailable {
        reason: format!("loader call for opt not yet wired (format={format})"),
    }
}

fn main() -> Result<()> {
    let _ctx = RecipeContext::new("inference_opt_smoke")?;
    let fixture = "tests/fixtures/architectures/opt/model.safetensors";
...
mod tests {
    ...
    fn loader_unavailable_when_path_exists_but_loader_unwired() {
        ...
        // to assert SmokeVerdict::Ok { family: "opt", .. }.
