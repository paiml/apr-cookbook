...
//!
//! Load a synthetic mamba micro-checkpoint via `aprender::rosetta`,
...
//! Provable-contract: contracts/inference-mamba-smoke-v1.yaml (grade A; lean_status: wip)
...
//!
//! Added by PMAT-300+ (architecture-demos: mamba coverage).
...
pub fn smoke(fixture_path: &str, format: &str) -> SmokeVerdict {
    ...
    SmokeVerdict::LoaderUnavailable {
        reason: format!("loader call for mamba not yet wired (format={format})"),
    }
}

fn main() -> Result<()> {
    let _ctx = RecipeContext::new("inference_mamba_smoke")?;
    let fixture = "tests/fixtures/architectures/mamba/model.safetensors";
...
mod tests {
    ...
    fn loader_unavailable_when_path_exists_but_loader_unwired() {
        ...
        // to assert SmokeVerdict::Ok { family: "mamba", .. }.
