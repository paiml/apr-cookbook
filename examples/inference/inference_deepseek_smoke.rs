...
//!
//! Load a synthetic deepseek micro-checkpoint via `aprender::rosetta`,
...
//! Provable-contract: contracts/inference-deepseek-smoke-v1.yaml (grade A; lean_status: wip)
...
//!
//! Added by PMAT-300+ (architecture-demos: deepseek coverage).
...
pub fn smoke(fixture_path: &str, format: &str) -> SmokeVerdict {
    ...
    SmokeVerdict::LoaderUnavailable {
        reason: format!("loader call for deepseek not yet wired (format={format})"),
    }
}

fn main() -> Result<()> {
    let _ctx = RecipeContext::new("inference_deepseek_smoke")?;
    let fixture = "tests/fixtures/architectures/deepseek/model.safetensors";
...
mod tests {
    ...
    fn loader_unavailable_when_path_exists_but_loader_unwired() {
        ...
        // to assert SmokeVerdict::Ok { family: "deepseek", .. }.
