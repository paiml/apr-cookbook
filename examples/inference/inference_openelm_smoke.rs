...
//!
//! Load a synthetic openelm micro-checkpoint via `aprender::rosetta`,
...
//! Provable-contract: contracts/inference-openelm-smoke-v1.yaml (grade A; lean_status: wip)
//! Citation: Mehta et al. (2024). OpenELM: An Efficient Language Model Family with Open Training and Inference Framework. arXiv:2404.14619
//!
//! Run with: cargo run --example inference_openelm_smoke
//!
//! Added by PMAT-300+ (architecture-demos: openelm coverage).
...
pub fn smoke(fixture_path: &str, format: &str) -> SmokeVerdict {
    ...
    SmokeVerdict::LoaderUnavailable {
        reason: format!("loader call for openelm not yet wired (format={format})"),
    }
}

fn main() -> Result<()> {
    let _ctx = RecipeContext::new("inference_openelm_smoke")?;
    let fixture = "tests/fixtures/architectures/openelm/model.safetensors";
...
mod tests {
    ...
    fn loader_unavailable_when_path_exists_but_loader_unwired() {
        ...
        // to assert SmokeVerdict::Ok { family: "openelm", .. }.
