...
//!
//! Load the bundled Gemma micro-config, validate `query_pre_attn_scalar`
//! is present (Gemma's discriminator vs Llama — Gemma scales queries
//! before attention), and run a deterministic seeded forward simulation.
//!
//! Demonstrates the **GEMMA.smoke** recipe per
//! `docs/specifications/architecture-demos.md` for the Gemma family
//! (`GemmaForCausalLM`, `Gemma2ForCausalLM`, `Gemma3ForCausalLM`).
//!
//! IIUR Contract: contracts/recipe-iiur-v1.yaml
//! Provable-contract: contracts/inference-gemma-smoke-v1.yaml (grade C; lean_status: wip)
//! Citation: Gemma Team (2024). Gemma. arXiv:2403.08295
//!
//! Run with: cargo run --example inference_gemma_smoke
//!
//! Added by PMAT-305 (architecture-demos: gemma family).
...
const FAMILY: &str = "gemma";
const FIXTURE_CONFIG: &str = "tests/fixtures/architectures/gemma/config.json";

fn forward_sim(seed: u64, vocab_size: u32, hidden_size: u32, query_scalar: u32) -> u32 {
    let mut state = seed | 1;
    let mut acc: u32 = query_scalar;
...
pub fn smoke(fixture_path: &str, format: &str) -> SmokeVerdict {
    ...
    let scalar = match extract_number(&body, "query_pre_attn_scalar") {
        Some(n) => n as u32,
        None => return missing("query_pre_attn_scalar (Gemma discriminator)"),
    };
    let checksum = forward_sim(42, vocab_size, hidden_size, scalar);
    SmokeVerdict::Ok {
        family: FAMILY.to_string(),
        format: format.to_string(),
        logits_checksum: checksum,
        layer_count: num_layers,
        query_scalar_x10: scalar * 10,
    }
}
...
mod tests {
    ...
    #[test]
    fn happy_path_returns_ok_gemma() {
    ...
    #[test]
    fn happy_path_query_scalar_present() {
        if let SmokeVerdict::Ok {
            query_scalar_x10, ..
        } = smoke(FIXTURE_CONFIG, "safetensors")
        {
            assert_eq!(query_scalar_x10, 160); // 16 * 10
        }
    }
    ...
    #[test]
    fn query_scalar_affects_checksum() {
