...
//!
//! Load a synthetic llama micro-checkpoint via `aprender::rosetta`,
...
//!
//! Added by PMAT-300+ (architecture-demos: llama coverage).
...
pub fn smoke(fixture_path: &str, format: &str) -> SmokeVerdict {
    ...
    SmokeVerdict::LoaderUnavailable {
        reason: format!("loader call for llama not yet wired (format={format})"),
    }
}

fn main() -> Result<()> {
    let _ctx = RecipeContext::new("inference_llama_smoke")?;
    let fixture = "tests/fixtures/architectures/llama/model.safetensors";
...
mod tests {
    ...
    fn loader_unavailable_when_path_exists_but_loader_unwired() {
        ...
        // to assert SmokeVerdict::Ok { family: "llama", .. }.
