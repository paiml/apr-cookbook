//! # Tier 2.1 — LoRA rank-8 on gemma
//!
//! Falsifier: rank-8 LoRA merge round-trip is bit-identical when α/r = 1.0
//! (and trainable param count = r × (d_in + d_out)).
//!
//! Run with: cargo run --example t2_lora_rank8_gemma
...
const ALPHA: f64 = 8.0; // α=r so scale = 1.0 (round-trip identity)

fn main() -> Result<()> {
    let _ctx = RecipeContext::new("t2_lora_rank8_gemma")?;
