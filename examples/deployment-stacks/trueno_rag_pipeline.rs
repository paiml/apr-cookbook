//! # Trueno Rag Pipeline -- Sovereign deployment recipe loader
//!
//! Loads the deployment recipe `recipes/trueno-rag-pipeline.yaml`, validates its schema, and
...
//!
//! Run with: cargo run --example trueno_rag_pipeline
//!
...
const RECIPE_YAML: &str = include_str!("recipes/trueno-rag-pipeline.yaml");

fn main() -> Result<()> {
    let _ctx = RecipeContext::new("trueno_rag_pipeline")?;
