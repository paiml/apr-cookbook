//! # Batuta Agent -- Sovereign deployment recipe loader
//!
//! Loads the deployment recipe `recipes/batuta-agent.yaml`, validates its schema, and
...
//!
//! Run with: cargo run --example batuta_agent
//!
...
const RECIPE_YAML: &str = include_str!("recipes/batuta-agent.yaml");

fn main() -> Result<()> {
    let _ctx = RecipeContext::new("batuta_agent")?;
