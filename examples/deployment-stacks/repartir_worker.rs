//! # Repartir Worker -- Sovereign deployment recipe loader
//!
//! Loads the deployment recipe `recipes/repartir-worker.yaml`, validates its schema, and
...
//!
//! Run with: cargo run --example repartir_worker
//!
...
const RECIPE_YAML: &str = include_str!("recipes/repartir-worker.yaml");

fn main() -> Result<()> {
    let _ctx = RecipeContext::new("repartir_worker")?;
