//! # Apr Inference Server -- Sovereign deployment recipe loader
//!
//! Loads the deployment recipe `recipes/apr-inference-server.yaml`, validates its schema, and
...
//!
//! Run with: cargo run --example apr_inference_server
//!
...
const RECIPE_YAML: &str = include_str!("recipes/apr-inference-server.yaml");

fn main() -> Result<()> {
    let _ctx = RecipeContext::new("apr_inference_server")?;
