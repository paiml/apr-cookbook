-- Theorems for `contracts/inference-mamba-smoke-v1.yaml`.
--
-- MAMBA pure-SSM discriminator. The smoke recipe models `smoke()` and `forward_sim()`
-- as pure functions of their inputs; determinism and totality are
-- derivable from referential transparency.

namespace ProvableContracts.ArchitectureDemos.Mamba
...
end ProvableContracts.ArchitectureDemos.Mamba
