-- Theorems for `contracts/inference-deepseek-smoke-v1.yaml`.
--
-- DeepSeek MoE discriminator. The smoke recipe models `smoke()` and `forward_sim()`
-- as pure functions of their inputs; determinism and totality are
-- derivable from referential transparency.

namespace ProvableContracts.ArchitectureDemos.Deepseek
...
end ProvableContracts.ArchitectureDemos.Deepseek
