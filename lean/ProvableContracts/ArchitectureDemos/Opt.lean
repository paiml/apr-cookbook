-- Theorems for `contracts/inference-opt-smoke-v1.yaml`.
--
-- OPT Pre-LN discriminator. The smoke recipe models `smoke()` and `forward_sim()`
-- as pure functions of their inputs; determinism and totality are
-- derivable from referential transparency.

namespace ProvableContracts.ArchitectureDemos.Opt
...
end ProvableContracts.ArchitectureDemos.Opt
