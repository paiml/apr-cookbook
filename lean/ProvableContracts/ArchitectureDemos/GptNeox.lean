-- Theorems for `contracts/inference-gptneox-smoke-v1.yaml`.
--
-- GPT-NeoX use_parallel_residual discriminator. The smoke recipe models `smoke()` and `forward_sim()`
-- as pure functions of their inputs; determinism and totality are
-- derivable from referential transparency.

namespace ProvableContracts.ArchitectureDemos.GptNeox
...
end ProvableContracts.ArchitectureDemos.GptNeox
