-- Theorems for `contracts/inference-bert-smoke-v1.yaml`.
--
-- BERT encoder-only discriminator. The smoke recipe models `smoke()` and `forward_sim()`
-- as pure functions of their inputs; determinism and totality are
-- derivable from referential transparency.

namespace ProvableContracts.ArchitectureDemos.Bert
...
end ProvableContracts.ArchitectureDemos.Bert
