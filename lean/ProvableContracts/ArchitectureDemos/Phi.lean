-- Theorems for `contracts/inference-phi-smoke-v1.yaml`.
--
-- Phi-3 fused-QKV discriminator. The smoke recipe models `smoke()` and `forward_sim()`
-- as pure functions of their inputs; determinism and totality are
-- derivable from referential transparency.

namespace ProvableContracts.ArchitectureDemos.Phi
...
end ProvableContracts.ArchitectureDemos.Phi
