...
- **D3 Kani 0.9 → 1.0** — would require actual Kani CI runs (not just stub functions). After PMAT-320..323 every architecture-demos contract has full `#[kani::proof]` stub coverage: 69 of 69 declared harnesses implemented (3 pipeline + 15 meta + 51 family-smoke). Without an actual Kani CI run the 0.9 ceiling persists regardless of stub count, but `pv score` no longer flags any "(no harness)" or "(unmatched)" markers across the architecture-demos contract corpus. Current 0.9 already exceeds every other flagship contract's Kani score (mmap 0.83, avx512 not measured). Genuine 1.0 would require wiring up `cargo kani` to run in CI, which is a separate cross-spec effort.
