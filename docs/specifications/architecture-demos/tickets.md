...
| PMAT-322 | Kaizen — implement Rust `#[kani::proof]` stub functions for the 15 declared-but-unimplemented meta-contract harnesses (detector/summary/compare/quirk-audit/alias-resolver, 3 each) | landed |
| PMAT-323 | Kaizen — implement the remaining 51 family-smoke `#[kani::proof]` stubs (17 families × 3 each, including a moonshine third forward-determinism stub); closes the declared-but-unimplemented gap to 0 | landed |
...
Total architecture-demos contracts after PMAT-320: **24**, all at Grade A 0.98. Kani harness implementation count after PMAT-323: **69 of 69 declared** (3 pipeline + 15 meta + 51 family-smoke). Every harness referenced from the architecture-demos YAMLs has a corresponding `#[kani::proof]` Rust function in `kani/src/lib.rs`.
...
- ~~**D3 Kani 0.9 → 1.0**~~ — **Runtime witness shipped in PMAT-324** ([kani-ci.md](../kani-ci.md)). The 0.9 ceiling in `pv score` is by design — it reports "harnesses declared and implemented" (the `bounded_int` strategy's structural weight). The new `kani-gate` CI job installs Kani and runs `cargo kani` against the full 108-harness suite on every PR — when the badge is green, every obligation is symbolically verified, not just declared. This mirrors the Lean side: `pv score` reports D4 1.0 when `status: proved`; only the `lean-gate` badge attests that `lake build` actually compiles the proofs. **Score = static readiness. CI gate = runtime verification.** A genuine score-axis bump to 1.0 would require an `actually_verified` evidence field upstream in `aprender-contracts`, tracked as a backlog item in [kani-ci.md](../kani-ci.md#backlog).
...
- ~~**Kani harnesses declared but unimplemented**~~ — **DONE** in PMAT-322 + PMAT-323. All 69 architecture-demos harnesses now have `#[kani::proof]` Rust functions in `kani/src/lib.rs` (3 pipeline + 15 meta + 51 family-smoke). `pv score` shows '+ <obligation>: KANI-... [bounded_int]' for every obligation across the architecture-demos corpus; no `(no harness)` or `(unmatched)` markers anywhere.
- ~~**Cookbook-wide Kani CI integration**~~ — **DONE** in PMAT-324. New `kani-gate` job in `.github/workflows/ci.yml` installs Kani 0.67.0 and runs `cargo kani` against the standalone `kani/` crate on every PR. 108 harnesses (39 base + 69 architecture-demos) verify in ~14s on a stable laptop; CI cold-install adds ~3-5min. See [kani-ci.md](../kani-ci.md) for the full integration spec.
