...
4. **Provable-contract grade A (Invariant B) — Lean proofs MANDATORY**: every `status: certified` family ships a per-family provable-contract at `contracts/inference-<family>-smoke-v1.yaml` AND a corresponding Lean module at `lean/ProvableContracts/ArchitectureDemos/<Family>.lean` containing real proof bodies (no `sorry` placeholders) for each declared theorem. The contract's `proof_obligations[].lean.status` must be `proved` (NOT `wip` or `sorry`). `lake build` must succeed for the architecture-demos namespace; `pv score --summary` must report **grade A (≥ 0.93)** with `D4 Lean: 1.00`. Because architecture-demos recipes are pure functions of their inputs (config-parsing + LCG-seeded forward simulation), determinism + totality theorems are derivable in pure Lean (`rfl`, structural induction) — `sorry` is NOT an acceptable status for any architecture-demos theorem.
